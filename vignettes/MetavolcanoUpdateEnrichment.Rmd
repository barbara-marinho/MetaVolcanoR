...
- **Customizable feature barplot** colors via new `colors` parameter in 
  `draw_featurebar()`
- **`enrichment_mv()`**: new function for feature set enrichment analysis 
...
When `label_top_n` is used, features are now ranked by the topconfects 
...
```{r rem_custom_colors, fig.width=11, fig.height=8, out.width="100%"}
...
DE). This could label features with inconsistent direction across studies.
